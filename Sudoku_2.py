...
def SudukoHint(arr):
    ...
    for i in range(rows):
        for j in range(columns):
            ...
            if arr[i][j] != -1:
                cell_no[cell_no].hints = []  # Set hints to empty if solution exits
    ...
    for i in range(rows):
        for j in range(columns):
            if cell_list[cell_no].value != -1:  # If solution exits skip hint
                continue
            grid_no = ((i / 3) * 3) + (j / 3)
    ...
    for cell_no in range(max):
        ...
        if cell_list[cell_no].value == -1 and len(cell_list[cell_no].hints) != 1:
            print chr(row), i + 1,
...
def is_valid(arr):
    row = len(arr)
    columns = len(arr)
    if not is_board_size_correct(arr):
        return False
    for i in range(row):
        for j in range(columns):
            cell_value = arr[i][j]
...
# SudukoHint(arr)
def main(arr):
    return SudukoHint(cell_list)

    # board size
    # input should be between 1 and 9 and '.'
    # check corners like A1
