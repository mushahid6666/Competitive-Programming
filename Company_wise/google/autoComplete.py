...
sentence (at least one word and end with a special character '#'). For each character they
...
class TrieNode(object):
    def __init__(self, char):
        self.node_char = char
        self.child_nodes =  [None] * 128
        self.top_sentenses = []
        self.all_sentenses = set()
...
class AutocompleteSystem(object):
    ...
    def update_frequent_nodes(self, node, cur_sentense, frequency):
        if cur_sentense in node.all_sentenses:
            for i in range(len(node.top_sentenses)):
                if cur_sentense == node.top_sentenses[i][1]:
                    node.top_sentenses[i][0] += 1
        else:
            node.top_sentenses.append([frequency, cur_sentense])
            node.all_sentenses.add(cur_sentense)
        node.top_sentenses.sort(self.compare)



    def constructTrie(self, sentences, times, root_node):

        for i in range(len(sentences)):
            cur_sentense = sentences[i]
            frequency = times[i]
            cur_node = root_node
            for i in range(len(cur_sentense)):
                cur_charecter = cur_sentense[i]
                if cur_node.child_nodes[ord(cur_charecter)] == None:
                    new_child_node = TrieNode(cur_charecter)
                    cur_node.child_nodes[ord(cur_charecter)] = new_child_node
                    cur_node = new_child_node
                else:
                    cur_node  = cur_node.child_nodes[ord(cur_charecter)]
                self.update_frequent_nodes(cur_node, cur_sentense, frequency)
    ...
    def __init__(self, sentences, times):
        ...
        self.cur_search_node = self.root_node
        self.cur_search_sentense = ""
        if len(sentences) == 0:
            return
        self.constructTrie(sentences, times, self.root_node)
    ...
    def input(self, c):
        ...
        """
        if c=="#":
            self.constructTrie([self.cur_search_sentense], [1], self.root_node)
            self.cur_search_node = self.root_node
            self.cur_search_sentense = ""
            return []
        self.cur_search_sentense += c
        if self.cur_search_node == None:
            return []
        if self.cur_search_node.child_nodes[ord(c)] == None:
            self.cur_search_node = None
            return []
        else:
            self.cur_search_node = self.cur_search_node.child_nodes[ord(c)]
            n = len(self.cur_search_node.top_sentenses) if len(self.cur_search_node.top_sentenses) <= 3 else 3
            result = [self.cur_search_node.top_sentenses[i][1] for i in range(n)]
